...
public class WindowsOSFileStore extends AbstractOSFileStore
{
    ...
    public boolean updateAttributes() {
        ...
        if (volumes.isEmpty()) {
            final String nameToMatch = (this.getMount().length() < 2) ? null : this.getMount().substring(0, 2);
            volumes = WindowsFileSystem.getWmiVolumes(nameToMatch, false);
        }
        for (final OSFileStore fileStore : volumes) {
