...
public class WindowsInternetProtocolStats extends AbstractInternetProtocolStats
{
    ...
    private static List<InternetProtocolStats.IPConnection> queryTCPv4Connections() {
        ...
        WindowsInternetProtocolStats.IPHLP.GetExtendedTcpTable((Pointer)null, sizePtr, false, 2, 5, 0);
        int size;
        Memory buf;
        do {
            size = sizePtr.getValue();
            buf = new Memory((long)size);
            WindowsInternetProtocolStats.IPHLP.GetExtendedTcpTable((Pointer)buf, sizePtr, false, 2, 5, 0);
        } while (size < sizePtr.getValue());
        final IPHlpAPI.MIB_TCPTABLE_OWNER_PID tcpTable = new IPHlpAPI.MIB_TCPTABLE_OWNER_PID((Pointer)buf);
    ...
    private static List<InternetProtocolStats.IPConnection> queryTCPv6Connections() {
        final List<InternetProtocolStats.IPConnection> conns = new ArrayList<InternetProtocolStats.IPConnection>();
        final IntByReference sizePtr = new IntByReference();
        WindowsInternetProtocolStats.IPHLP.GetExtendedTcpTable((Pointer)null, sizePtr, false, 23, 5, 0);
        int size;
        Memory buf;
        do {
            size = sizePtr.getValue();
            buf = new Memory((long)size);
            WindowsInternetProtocolStats.IPHLP.GetExtendedTcpTable((Pointer)buf, sizePtr, false, 23, 5, 0);
        } while (size < sizePtr.getValue());
        final IPHlpAPI.MIB_TCP6TABLE_OWNER_PID tcpTable = new IPHlpAPI.MIB_TCP6TABLE_OWNER_PID((Pointer)buf);
        for (int i = 0; i < tcpTable.dwNumEntries; ++i) {
    ...
    private static List<InternetProtocolStats.IPConnection> queryUDPv4Connections() {
        final List<InternetProtocolStats.IPConnection> conns = new ArrayList<InternetProtocolStats.IPConnection>();
        final IntByReference sizePtr = new IntByReference();
        WindowsInternetProtocolStats.IPHLP.GetExtendedUdpTable((Pointer)null, sizePtr, false, 2, 1, 0);
        int size;
        Memory buf;
        do {
            size = sizePtr.getValue();
            buf = new Memory((long)size);
            WindowsInternetProtocolStats.IPHLP.GetExtendedUdpTable((Pointer)buf, sizePtr, false, 2, 1, 0);
        } while (size < sizePtr.getValue());
        final IPHlpAPI.MIB_UDPTABLE_OWNER_PID udpTable = new IPHlpAPI.MIB_UDPTABLE_OWNER_PID((Pointer)buf);
    ...
    private static List<InternetProtocolStats.IPConnection> queryUDPv6Connections() {
        final List<InternetProtocolStats.IPConnection> conns = new ArrayList<InternetProtocolStats.IPConnection>();
        final IntByReference sizePtr = new IntByReference();
        WindowsInternetProtocolStats.IPHLP.GetExtendedUdpTable((Pointer)null, sizePtr, false, 23, 1, 0);
        int size;
        Memory buf;
        do {
            size = sizePtr.getValue();
            buf = new Memory((long)size);
            WindowsInternetProtocolStats.IPHLP.GetExtendedUdpTable((Pointer)buf, sizePtr, false, 23, 1, 0);
        } while (size < sizePtr.getValue());
        final IPHlpAPI.MIB_UDP6TABLE_OWNER_PID udpTable = new IPHlpAPI.MIB_UDP6TABLE_OWNER_PID((Pointer)buf);
        for (int i = 0; i < udpTable.dwNumEntries; ++i) {
