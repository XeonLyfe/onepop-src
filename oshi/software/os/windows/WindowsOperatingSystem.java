...
public class WindowsOperatingSystem extends AbstractOperatingSystem
{
    ...
    public WindowsOperatingSystem() {
        this.processMapFromRegistry = (Supplier<Map<Integer, ProcessPerformanceData.PerfCounterBlock>>)Memoizer.memoize(WindowsOperatingSystem::queryProcessMapFromRegistry, Memoizer.defaultExpiration());
        this.processMapFromPerfCounters = (Supplier<Map<Integer, ProcessPerformanceData.PerfCounterBlock>>)Memoizer.memoize(WindowsOperatingSystem::queryProcessMapFromPerfCounters, Memoizer.defaultExpiration());
        this.threadMapFromRegistry = (Supplier<Map<Integer, ThreadPerformanceData.PerfCounterBlock>>)Memoizer.memoize(WindowsOperatingSystem::queryThreadMapFromRegistry, Memoizer.defaultExpiration());
        this.threadMapFromPerfCounters = (Supplier<Map<Integer, ThreadPerformanceData.PerfCounterBlock>>)Memoizer.memoize(WindowsOperatingSystem::queryThreadMapFromPerfCounters, Memoizer.defaultExpiration());
    }
    ...
    private static long querySystemBootTime() {
        ...
        if (eventLog != null) {
            try {
                final Advapi32Util.EventLogIterator iter = new Advapi32Util.EventLogIterator((String)null, eventLog, 8);
                ...
                WindowsOperatingSystem.LOG.warn("Can't open event log \"{}\".", (Object)eventLog);
            }
        }
    ...
    public OSService[] getServices() {
        try (final W32ServiceManager sm = new W32ServiceManager()) {
            sm.open(4);
            final Winsvc.ENUM_SERVICE_STATUS_PROCESS[] services = sm.enumServicesStatusExProcess(48, 3, (String)null);
            final OSService[] svcArray = new OSService[services.length];
            ...
            WindowsOperatingSystem.LOG.error("Win32Exception: {}", (Object)ex.getMessage());
    ...
    private static String querySystemLog() {
        ...
        final WinNT.HANDLE h = Advapi32.INSTANCE.OpenEventLog((String)null, systemLog);
        if (h == null) {
            WindowsOperatingSystem.LOG.warn("Unable to open configured system Event log \"{}\". Calculating boot time from uptime.", (Object)systemLog);
            return null;
        }
        return systemLog;
    }
    ...
    static {
        LOG = LoggerFactory.getLogger((Class)WindowsOperatingSystem.class);
