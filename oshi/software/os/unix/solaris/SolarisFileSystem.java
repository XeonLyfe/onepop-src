...
public class SolarisFileSystem extends AbstractFileSystem
{
    ...
    public long getOpenFileDescriptors() {
        try (final KstatUtil.KstatChain kc = KstatUtil.openChain()) {
            ...
            if (ksp != null && KstatUtil.KstatChain.read(ksp)) {
                try (final KstatUtil.KstatChain kc = (KstatUtil.KstatChain)KstatUtil.dataLookupLong(ksp, "buf_inuse")) {}
                return;
            }
        }
        return 0L;
    ...
    public long getMaxFileDescriptors() {
        try (final KstatUtil.KstatChain kc = KstatUtil.openChain()) {
            final LibKstat.Kstat ksp = KstatUtil.KstatChain.lookup(null, -1, "file_cache");
            if (ksp != null && KstatUtil.KstatChain.read(ksp)) {
                try (final KstatUtil.KstatChain kc = (KstatUtil.KstatChain)KstatUtil.dataLookupLong(ksp, "buf_max")) {}
                return;
            }
        }
        return 0L;
