...
public class FreeBsdOSProcess extends AbstractOSProcess
{
    ...
    public FreeBsdOSProcess(final int pid, final String[] split) {
        super(pid);
    ...
    public List<OSThread> getThreadDetails() {
        ...
        for (final String thread : threadList) {
            final String[] split = ParseUtil.whitespaces.split(thread.trim(), 12);
            if (split.length == 10) {
    ...
    public boolean updateAttributes() {
        final String psCommand = "ps -awwxo state,pid,ppid,user,uid,group,gid,nlwp,pri,vsz,rss,etimes,systime,time,comm,majflt,minflt,args -p " + this.getProcessID();
        final List<String> procList = ExecutingCommand.runNative(psCommand);
        if (procList.size() > 1) {
            final String[] split = ParseUtil.whitespaces.split(procList.get(1).trim(), 18);
            if (split.length == 18) {
    ...
    private boolean updateAttributes(final String[] split) {
        ...
        final long nonVoluntaryContextSwitches = ParseUtil.parseLongOrDefault(split[17], 0L);
        final long voluntaryContextSwitches = ParseUtil.parseLongOrDefault(split[18], 0L);
