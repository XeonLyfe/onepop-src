...
public class LinuxOperatingSystem extends AbstractOperatingSystem
{
    ...
    public int getThreadCount() {
        try {
            ...
            LinuxOperatingSystem.LOG.error("Failed to get procs from sysinfo. {}", (Object)e.getMessage());
            return 0;
        }
    }
    ...
    private static Triplet<String, String, String> readOsRelease() {
        ...
        for (String line : osRelease) {
            if (line.startsWith("VERSION=")) {
                LinuxOperatingSystem.LOG.debug("os-release: {}", (Object)line);
    ...
    private static Triplet<String, String, String> execLsbRelease() {
        ...
        for (String line : ExecutingCommand.runNative("lsb_release -a")) {
            if (line.startsWith("Description:")) {
                LinuxOperatingSystem.LOG.debug("lsb_release -a: {}", (Object)line);
    ...
    private static Triplet<String, String, String> readLsbRelease() {
        ...
        for (String line : osRelease) {
            if (line.startsWith("DISTRIB_DESCRIPTION=")) {
                LinuxOperatingSystem.LOG.debug("lsb-release: {}", (Object)line);
    ...
    private static Triplet<String, String, String> readDistribRelease(final String filename) {
        if (new File(filename).exists()) {
            ...
            for (final String line : osRelease) {
                LinuxOperatingSystem.LOG.debug("{}: {}", (Object)filename, (Object)line);
                if (line.contains(" release ")) {
    ...
    public OSService[] getServices() {
        ...
        for (final String str : systemctl) {
            ...
            if (split.length >= 2 && split[0].endsWith(".service") && "enabled".equals(split[1])) {
                name = split[0].substring(0, split[0].length() - 8);
                final int index = name.lastIndexOf(46);
                final String shortName = (index < 0 || index > name.length() - 2) ? name : name.substring(index + 1);
        ...
        if (!systemctlFound) {
            ...
            if (dir.exists() && dir.isDirectory()) {
                for (final File f : dir.listFiles((f, name) -> name.toLowerCase().endsWith(".conf"))) {
                    final String name2 = f.getName().substring(0, f.getName().length() - 5);
                    final int index2 = name2.lastIndexOf(46);
                    final String shortName2 = (index2 < 0 || index2 > name2.length() - 2) ? name2 : name2.substring(index2 + 1);
                    if (!running.contains(name2) && !running.contains(shortName2)) {
                        final OSService s3 = new OSService(name2, 0, OSService.State.STOPPED);
                        services.add(s3);
                    }
                }
            }
    ...
    static {
        LOG = LoggerFactory.getLogger((Class)LinuxOperatingSystem.class);
