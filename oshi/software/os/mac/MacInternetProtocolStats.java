...
public class MacInternetProtocolStats extends AbstractInternetProtocolStats
{
    ...
    public MacInternetProtocolStats(final boolean elevated) {
        this.establishedv4v6 = (Supplier<Pair<Long, Long>>)Memoizer.memoize(NetStat::queryTcpnetstat, Memoizer.defaultExpiration());
    ...
    public InternetProtocolStats.TcpStats getTCPv4Stats() {
        ...
        if (this.isElevated) {
            return new InternetProtocolStats.TcpStats((long)this.establishedv4v6.get().getA(), ParseUtil.unsignedIntToLong(tcp.tcps_connattempt), ParseUtil.unsignedIntToLong(tcp.tcps_accepts), ParseUtil.unsignedIntToLong(tcp.tcps_conndrops), ParseUtil.unsignedIntToLong(tcp.tcps_drops), ParseUtil.unsignedIntToLong(tcp.tcps_sndpack), ParseUtil.unsignedIntToLong(tcp.tcps_rcvpack), ParseUtil.unsignedIntToLong(tcp.tcps_sndrexmitpack), ParseUtil.unsignedIntToLong(tcp.tcps_rcvbadsum + tcp.tcps_rcvbadoff + tcp.tcps_rcvmemdrop + tcp.tcps_rcvshort), 0L);
        }
        final CLibrary.BsdIpstat ip = this.ipstat.get();
        final CLibrary.BsdUdpstat udp = this.udpstat.get();
        return new InternetProtocolStats.TcpStats((long)this.establishedv4v6.get().getA(), ParseUtil.unsignedIntToLong(tcp.tcps_connattempt), ParseUtil.unsignedIntToLong(tcp.tcps_accepts), ParseUtil.unsignedIntToLong(tcp.tcps_conndrops), ParseUtil.unsignedIntToLong(tcp.tcps_drops), Math.max(0L, ParseUtil.unsignedIntToLong(ip.ips_delivered - udp.udps_opackets)), Math.max(0L, ParseUtil.unsignedIntToLong(ip.ips_total - udp.udps_ipackets)), ParseUtil.unsignedIntToLong(tcp.tcps_sndrexmitpack), Math.max(0L, ParseUtil.unsignedIntToLong(ip.ips_badsum + ip.ips_tooshort + ip.ips_toosmall + ip.ips_badhlen + ip.ips_badlen - udp.udps_hdrops + udp.udps_badsum + udp.udps_badlen)), 0L);
    }
    ...
    public InternetProtocolStats.TcpStats getTCPv6Stats() {
        ...
        final CLibrary.BsdUdpstat udp = this.udpstat.get();
        return new InternetProtocolStats.TcpStats((long)this.establishedv4v6.get().getB(), 0L, 0L, 0L, 0L, ip6.ip6s_localout - ParseUtil.unsignedIntToLong(udp.udps_snd6_swcsum), ip6.ip6s_total - ParseUtil.unsignedIntToLong(udp.udps_rcv6_swcsum), 0L, 0L, 0L);
    }
    ...
    private static List<Integer> queryFdList(final int pid) {
        ...
        int bufferSize = SystemB.INSTANCE.proc_pidinfo(pid, 1, 0L, (Structure)null, 0);
        if (bufferSize > 0) {
            ...
            bufferSize = SystemB.INSTANCE.proc_pidinfo(pid, 1, 0L, (Structure)fdArray[0], bufferSize);
