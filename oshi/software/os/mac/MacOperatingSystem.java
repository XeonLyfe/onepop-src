...
public class MacOperatingSystem extends AbstractOperatingSystem
{
    ...
    private String parseCodeName() {
        ...
        if (Util.isBlank(codeName)) {
            MacOperatingSystem.LOG.warn("Unable to parse version {}.{} to a codename.", (Object)this.major, (Object)this.minor);
        }
        return codeName;
    }
    ...
    public int getThreadCount() {
        ...
        for (int i = 0; i < numberOfProcesses; ++i) {
            final int exit = SystemB.INSTANCE.proc_pidinfo(pids[i], 4, 0L, (Structure)taskInfo, taskInfo.size());
            if (exit != -1) {
    ...
    public OSService[] getServices() {
        ...
        for (final File f : files) {
            name = f.getName().substring(0, f.getName().length() - 6);
    ...
    static {
        LOG = LoggerFactory.getLogger((Class)MacOperatingSystem.class);
