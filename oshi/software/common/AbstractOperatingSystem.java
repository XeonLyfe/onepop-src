...
public abstract class AbstractOperatingSystem implements OperatingSystem
{
    ...
    public AbstractOperatingSystem() {
        ...
        this.familyVersionInfo = (Supplier<Pair<String, OSVersionInfo>>)Memoizer.memoize(this::queryFamilyVersionInfo);
    ...
    public String getFamily() {
        return (String)this.familyVersionInfo.get().getA();
    }
    
    @Override
    public OSVersionInfo getVersionInfo() {
        return (OSVersionInfo)this.familyVersionInfo.get().getB();
    }
    ...
    private int queryPlatformBitness() {
        ...
        final int jvmBitness = (System.getProperty("os.arch").indexOf("64") != -1) ? 64 : 32;
    ...
    public List<OSProcess> getChildProcesses(final int parentPid, final Predicate<OSProcess> filter, final Comparator<OSProcess> sort, final int limit) {
        ...
        return this.queryChildProcesses(parentPid).stream().filter((Predicate<? super Object>)((filter == null) ? ProcessFiltering.ALL_PROCESSES : filter)).filter(p -> p.getProcessID() != parentPid && p.getStartTime() >= parentStartTime).sorted((Comparator<? super Object>)((sort == null) ? ProcessSorting.NO_SORTING : sort)).limit((limit > 0) ? limit : Long.MAX_VALUE).collect((Collector<? super Object, ?, List<OSProcess>>)Collectors.toList());
    }
    ...
    private static Set<Integer> getChildren(final Map<Integer, Integer> parentPidMap, final int parentPid) {
        return parentPidMap.entrySet().stream().filter(e -> e.getValue().equals(parentPid) && !e.getKey().equals(parentPid)).map((Function<? super Object, ?>)Map.Entry::getKey).collect((Collector<? super Object, ?, Set<Integer>>)Collectors.toSet());
    }
