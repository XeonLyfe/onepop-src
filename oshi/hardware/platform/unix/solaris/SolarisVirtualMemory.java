...
final class SolarisVirtualMemory extends AbstractVirtualMemory
{
    ...
    SolarisVirtualMemory(final SolarisGlobalMemory solarisGlobalMemory) {
        this.availTotal = (Supplier<Pair<Long, Long>>)Memoizer.memoize(SystemPages::queryAvailableTotal, Memoizer.defaultExpiration());
        this.usedTotal = (Supplier<Pair<Long, Long>>)Memoizer.memoize(SolarisVirtualMemory::querySwapInfo, Memoizer.defaultExpiration());
    ...
    public long getSwapUsed() {
        return (long)this.usedTotal.get().getA();
    }
    
    @Override
    public long getSwapTotal() {
        return (long)this.usedTotal.get().getB();
    }
    
    @Override
    public long getVirtualMax() {
        return this.global.getPageSize() * (long)this.availTotal.get().getB() + this.getSwapTotal();
    }
    
    @Override
    public long getVirtualInUse() {
        return this.global.getPageSize() * ((long)this.availTotal.get().getB() - (long)this.availTotal.get().getA()) + this.getSwapUsed();
    }
