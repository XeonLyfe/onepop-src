...
public final class MacHWDiskStore extends AbstractHWDiskStore
{
    ...
    private boolean updateDiskStats(final DiskArbitration.DASessionRef session, final Map<String, String> mountPointMap, final Map<CFKey, CoreFoundation.CFStringRef> cfKeyMap) {
        ...
        if (matchingDict != null) {
            final IOKit.IOIterator driveListIter = IOKitUtil.getMatchingServices((CoreFoundation.CFDictionaryRef)matchingDict);
                ...
                if (drive != null) {
                    if (drive.conformsTo("IOMedia")) {
                        ...
                        if (parent != null && (parent.conformsTo("IOBlockStorageDriver") || parent.conformsTo("AppleAPFSContainerScheme"))) {
                            ...
                            Pointer result = properties.getValue((PointerType)cfKeyMap.get(CFKey.STATISTICS));
                            final CoreFoundation.CFDictionaryRef statistics = new CoreFoundation.CFDictionaryRef(result);
                            this.timeStamp = System.currentTimeMillis();
                            result = statistics.getValue((PointerType)cfKeyMap.get(CFKey.READ_OPS));
                            final CoreFoundation.CFNumberRef stat = new CoreFoundation.CFNumberRef(result);
                            this.reads = stat.longValue();
                            result = statistics.getValue((PointerType)cfKeyMap.get(CFKey.READ_BYTES));
                            stat.setPointer(result);
                            this.readBytes = stat.longValue();
                            result = statistics.getValue((PointerType)cfKeyMap.get(CFKey.WRITE_OPS));
                            stat.setPointer(result);
                            this.writes = stat.longValue();
                            result = statistics.getValue((PointerType)cfKeyMap.get(CFKey.WRITE_BYTES));
                            stat.setPointer(result);
                            this.writeBytes = stat.longValue();
                            final Pointer readTimeResult = statistics.getValue((PointerType)cfKeyMap.get(CFKey.READ_TIME));
                            final Pointer writeTimeResult = statistics.getValue((PointerType)cfKeyMap.get(CFKey.WRITE_TIME));
                            ...
                            MacHWDiskStore.LOG.debug("Unable to find block storage driver properties for {}", (Object)bsdName);
                        }
                        ...
                        Pointer result2 = properties2.getValue((PointerType)cfKeyMap.get(CFKey.BSD_UNIT));
                        final CoreFoundation.CFNumberRef bsdUnit = new CoreFoundation.CFNumberRef(result2);
                        result2 = properties2.getValue((PointerType)cfKeyMap.get(CFKey.LEAF));
                        final CoreFoundation.CFBooleanRef cfFalse = new CoreFoundation.CFBooleanRef(result2);
                        final CoreFoundation.CFMutableDictionaryRef propertyDict = MacHWDiskStore.CF.CFDictionaryCreateMutable(MacHWDiskStore.CF.CFAllocatorGetDefault(), new CoreFoundation.CFIndex(0L), (Pointer)null, (Pointer)null);
                        propertyDict.setValue((PointerType)cfKeyMap.get(CFKey.BSD_UNIT), (PointerType)bsdUnit);
                        propertyDict.setValue((PointerType)cfKeyMap.get(CFKey.WHOLE), (PointerType)cfFalse);
                        matchingDict = MacHWDiskStore.CF.CFDictionaryCreateMutable(MacHWDiskStore.CF.CFAllocatorGetDefault(), new CoreFoundation.CFIndex(0L), (Pointer)null, (Pointer)null);
                        matchingDict.setValue((PointerType)cfKeyMap.get(CFKey.IO_PROPERTY_MATCH), (PointerType)propertyDict);
                        final IOKit.IOIterator serviceIterator = IOKitUtil.getMatchingServices((CoreFoundation.CFDictionaryRef)matchingDict);
                        properties2.release();
                        ...
                        if (serviceIterator != null) {
                            for (IOKit.IORegistryEntry sdService = IOKit.INSTANCE.IOIteratorNext(serviceIterator); sdService != null; sdService = IOKit.INSTANCE.IOIteratorNext(serviceIterator)) {
                                ...
                                if (disk != null) {
                                    ...
                                    if (diskInfo != null) {
                                        result2 = diskInfo.getValue((PointerType)cfKeyMap.get(CFKey.DA_MEDIA_NAME));
                                        final CoreFoundation.CFStringRef volumePtr = new CoreFoundation.CFStringRef(result2);
                                        type = volumePtr.stringValue();
                                        if (type == null) {
                                            type = "unknown";
                                        }
                                        result2 = diskInfo.getValue((PointerType)cfKeyMap.get(CFKey.DA_VOLUME_NAME));
                                        if (result2 == null) {
                                            name = type;
                                        }
                                        else {
                                            volumePtr.setPointer(result2);
                        ...
                        MacHWDiskStore.LOG.error("Unable to find IOMedia device or parent for {}", (Object)bsdName);
                    }
                    drive.release();
                }
    ...
    public static List<HWDiskStore> getDisks() {
        ...
        if (iter != null) {
            for (IOKit.IORegistryEntry media = iter.next(); media != null; media = iter.next()) {
                ...
                if (whole != null && whole) {
                    final DiskArbitration.DADiskRef disk = MacHWDiskStore.DA.DADiskCreateFromIOMedia(MacHWDiskStore.CF.CFAllocatorGetDefault(), session, (IOKit.IOObject)media);
        ...
        for (final String bsdName : bsdNames) {
            ...
            if (disk2 != null) {
                ...
                if (diskInfo != null) {
                    Pointer result = diskInfo.getValue((PointerType)cfKeyMap.get(CFKey.DA_DEVICE_MODEL));
                    final CoreFoundation.CFStringRef modelPtr = new CoreFoundation.CFStringRef(result);
                    model = modelPtr.stringValue();
                    if (model == null) {
                        model = "unknown";
                    }
                    result = diskInfo.getValue((PointerType)cfKeyMap.get(CFKey.DA_MEDIA_SIZE));
                    ...
                    if (!"Disk Image".equals(model)) {
                        ...
                        final CoreFoundation.CFMutableDictionaryRef propertyDict = MacHWDiskStore.CF.CFDictionaryCreateMutable(MacHWDiskStore.CF.CFAllocatorGetDefault(), new CoreFoundation.CFIndex(0L), (Pointer)null, (Pointer)null);
                        propertyDict.setValue((PointerType)cfKeyMap.get(CFKey.MODEL), (PointerType)modelNameRef);
                        final CoreFoundation.CFMutableDictionaryRef matchingDict = MacHWDiskStore.CF.CFDictionaryCreateMutable(MacHWDiskStore.CF.CFAllocatorGetDefault(), new CoreFoundation.CFIndex(0L), (Pointer)null, (Pointer)null);
                        matchingDict.setValue((PointerType)cfKeyMap.get(CFKey.IO_PROPERTY_MATCH), (PointerType)propertyDict);
                        final IOKit.IOIterator serviceIterator = IOKitUtil.getMatchingServices((CoreFoundation.CFDictionaryRef)matchingDict);
    ...
    static {
        ...
        LOG = LoggerFactory.getLogger((Class)MacHWDiskStore.class);
    }
