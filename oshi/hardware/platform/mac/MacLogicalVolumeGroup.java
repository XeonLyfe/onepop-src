        ...
        for (final String line : ExecutingCommand.runNative("diskutil cs list")) {
            if (line.contains("Logical Volume Group")) {
                ...
                if (lookForPVName) {
                    ...
                    logicalVolumesMap.computeIfAbsent(currentVolumeGroup, k -> new HashMap()).put((Object)line.substring(indexOf + 5).trim(), (Object)Collections.emptySet());
                }
            }
        }
        return logicalVolumesMap.entrySet().stream().map(e -> new MacLogicalVolumeGroup(e.getKey(), e.getValue(), physicalVolumesMap.get(e.getKey()))).collect((Collector<? super Object, ?, List<LogicalVolumeGroup>>)Collectors.toList());
    }
}
