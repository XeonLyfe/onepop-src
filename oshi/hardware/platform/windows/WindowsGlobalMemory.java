...
final class WindowsGlobalMemory extends AbstractGlobalMemory
{
    ...
    WindowsGlobalMemory() {
        this.availTotalSize = (Supplier<Triplet<Long, Long, Long>>)Memoizer.memoize(WindowsGlobalMemory::readPerfInfo, Memoizer.defaultExpiration());
        this.vm = Memoizer.memoize(this::createVirtualMemory);
    }
    
    @Override
    public long getAvailable() {
        return (long)this.availTotalSize.get().getA();
    }
    
    @Override
    public long getTotal() {
        return (long)this.availTotalSize.get().getB();
    }
    
    @Override
    public long getPageSize() {
        return (long)this.availTotalSize.get().getC();
    }
    ...
    static {
        LOG = LoggerFactory.getLogger((Class)WindowsGlobalMemory.class);
