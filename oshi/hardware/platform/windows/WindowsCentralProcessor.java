...
final class WindowsCentralProcessor extends AbstractCentralProcessor
{
    ...
    protected List<CentralProcessor.LogicalProcessor> initProcessorCounts() {
        if (VersionHelpers.IsWindows7OrGreater()) {
            final List<CentralProcessor.LogicalProcessor> logProcs = LogicalProcessorInformation.getLogicalProcessorInformationEx();
            int curNode = -1;
            int procNum = 0;
            int lp = 0;
            this.numaNodeProcToLogicalProcMap = new HashMap<String, Integer>();
            for (final CentralProcessor.LogicalProcessor logProc : logProcs) {
    ...
    private long[] queryNTPower(final int fieldIndex) {
        ...
        final Memory mem = new Memory((long)bufferSize);
        if (0 != PowrProf.INSTANCE.CallNtPowerInformation(11, (Pointer)null, 0, (Pointer)mem, bufferSize)) {
    ...
    static {
        LOG = LoggerFactory.getLogger((Class)WindowsCentralProcessor.class);
    }
}
