...
public final class WindowsPowerSource extends AbstractPowerSource
{
    ...
    private static WindowsPowerSource getPowerSource(final String name) {
        ...
        final Memory mem = new Memory((long)size);
        if (0 == PowrProf.INSTANCE.CallNtPowerInformation(5, (Pointer)null, 0, (Pointer)mem, size)) {
            final PowrProf.SystemBatteryState batteryState = new PowrProf.SystemBatteryState((Pointer)mem);
        ...
        final WinNT.HANDLE hdev = SetupApi.INSTANCE.SetupDiGetClassDevs(WindowsPowerSource.GUID_DEVCLASS_BATTERY, (Pointer)null, (Pointer)null, 18);
        if (WinBase.INVALID_HANDLE_VALUE != hdev) {
            ...
            for (int idev = 0; !batteryFound && idev < 100; ++idev) {
                ...
                did.cbSize = did.size();
                if (SetupApi.INSTANCE.SetupDiEnumDeviceInterfaces(hdev, (Pointer)null, WindowsPowerSource.GUID_DEVCLASS_BATTERY, idev, did)) {
                    ...
                    if (122 == Kernel32.INSTANCE.GetLastError()) {
                        final Memory pdidd = new Memory((long)requiredSize.getValue());
                        pdidd.setInt(0L, 4 + (WindowsPowerSource.X64 ? 4 : WindowsPowerSource.CHAR_WIDTH));
                        if (SetupApi.INSTANCE.SetupDiGetDeviceInterfaceDetail(hdev, did, (Pointer)pdidd, (int)pdidd.size(), requiredSize, (SetupApi.SP_DEVINFO_DATA)null)) {
                            final String devicePath = (WindowsPowerSource.CHAR_WIDTH > 1) ? pdidd.getWideString(4L) : pdidd.getString(4L);
                            final WinNT.HANDLE hBattery = Kernel32.INSTANCE.CreateFile(devicePath, -1073741824, 3, (WinBase.SECURITY_ATTRIBUTES)null, 3, 128, (WinNT.HANDLE)null);
                            if (!WinBase.INVALID_HANDLE_VALUE.equals((Object)hBattery)) {
    ...
    private static String batteryQueryString(final WinNT.HANDLE hBattery, final int tag, final int infoLevel) {
        ...
        do {
            ...
            ret = Kernel32.INSTANCE.DeviceIoControl(hBattery, 2703428, bqi.getPointer(), bqi.size(), (Pointer)nameBuf, (int)nameBuf.size(), dwOut, (Pointer)null);
