...
public final class PerfstatCpu
{
    ...
    public static Perfstat.perfstat_cpu_total_t queryCpuTotal() {
        ...
        final int ret = PerfstatCpu.PERF.perfstat_cpu_total((Perfstat.perfstat_id_t)null, cpu, cpu.size(), 1);
        if (ret > 0) {
            return cpu;
        }
    ...
    public static Perfstat.perfstat_cpu_t[] queryCpu() {
        ...
        final int cputotal = PerfstatCpu.PERF.perfstat_cpu((Perfstat.perfstat_id_t)null, (Perfstat.perfstat_cpu_t[])null, cpu.size(), 0);
        if (cputotal > 0) {
