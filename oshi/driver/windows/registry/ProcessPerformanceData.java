...
public final class ProcessPerformanceData
{
    ...
    public static Map<Integer, PerfCounterBlock> buildProcessMapFromRegistry(final Collection<Integer> pids) {
        ...
        for (final Map<ProcessInformation.ProcessPerformanceProperty, Object> processInstanceMap : processInstanceMaps) {
            final int pid = processInstanceMap.get(ProcessInformation.ProcessPerformanceProperty.PROCESSID);
            ...
            if ((pids == null || pids.contains(pid)) && !"_Total".equals(name)) {
                long ctime = processInstanceMap.get(ProcessInformation.ProcessPerformanceProperty.CREATIONDATE);
                if (ctime > now) {
                ...
                processMap.put(pid, new PerfCounterBlock(name, processInstanceMap.get(ProcessInformation.ProcessPerformanceProperty.PARENTPROCESSID), processInstanceMap.get(ProcessInformation.ProcessPerformanceProperty.PRIORITY), processInstanceMap.get(ProcessInformation.ProcessPerformanceProperty.PRIVATEPAGECOUNT), ctime, upTime, processInstanceMap.get(ProcessInformation.ProcessPerformanceProperty.READTRANSFERCOUNT), processInstanceMap.get(ProcessInformation.ProcessPerformanceProperty.WRITETRANSFERCOUNT), processInstanceMap.get(ProcessInformation.ProcessPerformanceProperty.PAGEFAULTSPERSEC)));
            }
        }
        return processMap;
    }
    ...
    public static Map<Integer, PerfCounterBlock> buildProcessMapFromPerfCounters(final Collection<Integer> pids) {
        ...
        final List<Long> pidList = valueMap.get(ProcessInformation.ProcessPerformanceProperty.PROCESSID);
        final List<Long> ppidList = valueMap.get(ProcessInformation.ProcessPerformanceProperty.PARENTPROCESSID);
        final List<Long> priorityList = valueMap.get(ProcessInformation.ProcessPerformanceProperty.PRIORITY);
        final List<Long> ioReadList = valueMap.get(ProcessInformation.ProcessPerformanceProperty.READTRANSFERCOUNT);
        final List<Long> ioWriteList = valueMap.get(ProcessInformation.ProcessPerformanceProperty.WRITETRANSFERCOUNT);
        final List<Long> workingSetSizeList = valueMap.get(ProcessInformation.ProcessPerformanceProperty.PRIVATEPAGECOUNT);
        final List<Long> creationTimeList = valueMap.get(ProcessInformation.ProcessPerformanceProperty.CREATIONDATE);
