...
public final class HkeyPerformanceDataUtil
{
    ...
    public static <T extends Enum<T> & PerfCounterWildcardQuery.PdhCounterWildcardProperty> Triplet<List<Map<T, Object>>, Long, Long> readPerfDataFromRegistry(final String objectName, final Class<T> counterEnum) {
            ...
            if (perfObject.ObjectNameTitleIndex == HkeyPerformanceDataUtil.COUNTER_INDEX_MAP.get(objectName)) {
                ...
                for (int inst = 0; inst < perfObject.NumInstances; ++inst) {
                    ...
                    for (int i = 1; i < counterKeys.length; ++i) {
                        ...
                        final int keyIndex = HkeyPerformanceDataUtil.COUNTER_INDEX_MAP.get(key.getCounter());
    ...
    private static <T extends Enum<T> & PerfCounterWildcardQuery.PdhCounterWildcardProperty> Pair<Integer, EnumMap<T, Integer>> getCounterIndices(final String objectName, final Class<T> counterEnum) {
        if (!HkeyPerformanceDataUtil.COUNTER_INDEX_MAP.containsKey(objectName)) {
            HkeyPerformanceDataUtil.LOG.debug("Couldn't find counter index of {}.", (Object)objectName);
            return null;
        }
        ...
        for (int i = 1; i < enumConstants.length; ++i) {
            ...
            final String counterName = key.getCounter();
            if (!HkeyPerformanceDataUtil.COUNTER_INDEX_MAP.containsKey(counterName)) {
                HkeyPerformanceDataUtil.LOG.debug("Couldn't find counter index of {}.", (Object)counterName);
                return null;
            }
    ...
    private static Memory readPerfDataBuffer(final String objectName) {
        ...
        if (ret != 0 && ret != 234) {
            HkeyPerformanceDataUtil.LOG.error("Error reading performance data from registry for {}.", (Object)objectName);
            return null;
        }
        while (ret == 234) {
            bufferSize += 4096;
    ...
    private static Map<String, Integer> mapCounterIndicesFromRegistry() {
        ...
        try {
            ...
            HkeyPerformanceDataUtil.LOG.error("Unable to locate English counter names in registry Perflib 009. Counters may need to be rebuilt: ", (Throwable)we);
        }
    ...
    static {
        LOG = LoggerFactory.getLogger((Class)HkeyPerformanceDataUtil.class);
