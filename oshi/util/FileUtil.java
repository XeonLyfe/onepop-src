...
public final class FileUtil
{
    ...
    public static List<String> readFile(final String filename, final boolean reportError) {
        if (new File(filename).canRead()) {
            if (FileUtil.LOG.isDebugEnabled()) {
                FileUtil.LOG.debug("Reading file {}", (Object)filename);
            }
            try {
                ...
                if (reportError) {
                    FileUtil.LOG.error("Error reading file {}. {}", (Object)filename, (Object)e.getMessage());
                }
                else {
                    FileUtil.LOG.debug("Error reading file {}. {}", (Object)filename, (Object)e.getMessage());
                }
                return new ArrayList<String>();
            }
        }
        if (reportError) {
            FileUtil.LOG.warn("File not found or not readable: {}", (Object)filename);
        }
        return new ArrayList<String>();
    }
    ...
    public static long getLongFromFile(final String filename) {
        ...
        if (!read.isEmpty()) {
            if (FileUtil.LOG.isTraceEnabled()) {
                FileUtil.LOG.trace("Read {}", (Object)read.get(0));
            }
    ...
    public static int getIntFromFile(final String filename) {
        ...
        try {
            ...
            FileUtil.LOG.warn("Unable to read value from {}. {}", (Object)filename, (Object)ex.getMessage());
        }
        return 0;
    }
    ...
    public static Properties readPropertiesFromFilename(final String propsFilename) {
        ...
        for (final ClassLoader loader : Stream.of(new ClassLoader[] { Thread.currentThread().getContextClassLoader(), ClassLoader.getSystemClassLoader(), FileUtil.class.getClassLoader() }).collect((Collector<? super ClassLoader, ?, LinkedHashSet>)Collectors.toCollection((Supplier<R>)LinkedHashSet::new))) {
    ...
    private static boolean readPropertiesFromClassLoader(final String propsFilename, final Properties archProps, final ClassLoader loader) {
        ...
        try {
            ...
            if (resources.isEmpty()) {
                FileUtil.LOG.debug("No {} file found from ClassLoader {}", (Object)propsFilename, (Object)loader);
                return false;
            }
            if (resources.size() > 1) {
                FileUtil.LOG.warn("Configuration conflict: there is more than one {} file on the classpath", (Object)propsFilename);
                return true;
            }
            try (final InputStream in = resources.get(0).openStream()) {
                if (in != null) {
                    archProps.load(in);
                }
            }
