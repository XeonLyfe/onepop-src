...
public final class PerfDataUtil
{
    ...
    public static long updateQueryTimestamp(final WinNT.HANDLEByReference query) {
        ...
        if (ret != 0) {
            if (PerfDataUtil.LOG.isWarnEnabled()) {
                PerfDataUtil.LOG.warn("Failed to update counter. Error code: {}", (Object)String.format(FormatUtil.formatError(ret), new Object[0]));
            }
            return 0L;
        }
    ...
    public static boolean openQuery(final WinNT.HANDLEByReference q) {
        final int ret = PerfDataUtil.PDH.PdhOpenQuery((String)null, PerfDataUtil.PZERO, q);
        if (ret != 0) {
            if (PerfDataUtil.LOG.isErrorEnabled()) {
                PerfDataUtil.LOG.error("Failed to open PDH Query. Error code: {}", (Object)String.format(FormatUtil.formatError(ret), new Object[0]));
            }
            return false;
        }
        return true;
    ...
    public static long queryCounter(final WinNT.HANDLEByReference counter) {
        ...
        if (ret != 0) {
            if (PerfDataUtil.LOG.isWarnEnabled()) {
                PerfDataUtil.LOG.warn("Failed to get counter. Error code: {}", (Object)String.format(FormatUtil.formatError(ret), new Object[0]));
            }
            return ret;
        }
    ...
    public static boolean addCounter(final WinNT.HANDLEByReference query, final String path, final WinNT.HANDLEByReference p) {
        ...
        if (ret != 0) {
            if (PerfDataUtil.LOG.isWarnEnabled()) {
                PerfDataUtil.LOG.warn("Failed to add PDH Counter: {}, Error code: {}", (Object)path, (Object)String.format(FormatUtil.formatError(ret), new Object[0]));
            }
            return false;
        }
        return true;
    ...
    static {
        LOG = LoggerFactory.getLogger((Class)PerfDataUtil.class);
