...
public final class PerfCounterQueryHandler implements AutoCloseable
{
    ...
    public boolean addCounterToQuery(final PerfDataUtil.PerfCounter counter) {
        if (this.queryHandle == null) {
            ...
            if (!PerfDataUtil.openQuery(this.queryHandle)) {
                PerfCounterQueryHandler.LOG.warn("Failed to open a query for PDH object: {}", (Object)counter.getObject());
                this.queryHandle = null;
                return false;
            }
        }
        ...
        if (!PerfDataUtil.addCounter(this.queryHandle, counter.getCounterPath(), p)) {
            PerfCounterQueryHandler.LOG.warn("Failed to add counter for PDH object: {}", (Object)counter.getObject());
            return false;
        }
    ...
    public long queryCounter(final PerfDataUtil.PerfCounter counter) {
        if (!this.counterHandleMap.containsKey(counter)) {
            if (PerfCounterQueryHandler.LOG.isWarnEnabled()) {
                PerfCounterQueryHandler.LOG.warn("Counter {} does not exist to query.", (Object)counter.getCounterPath());
            }
            return 0L;
        }
        ...
        if (value < 0L) {
            if (PerfCounterQueryHandler.LOG.isWarnEnabled()) {
                PerfCounterQueryHandler.LOG.warn("Error querying counter {}: {}", (Object)counter.getCounterPath(), (Object)String.format(FormatUtil.formatError((int)value), new Object[0]));
            }
            return 0L;
        }
        return value;
    ...
    static {
        LOG = LoggerFactory.getLogger((Class)PerfCounterQueryHandler.class);
    }
}
